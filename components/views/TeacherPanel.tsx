...
import { PencilIcon, UserGroupIcon, PlusIcon, TrashIcon, ClipboardListIcon, AcademicCapIcon, CalendarIcon, CheckIcon, DownloadIcon, MailIcon, BookOpenIcon, HomeIcon, ChatIcon, SearchIcon, CurrencyDollarIcon, CreditCardIcon, XIcon, CheckCircleIcon, ChevronLeftIcon } from '../Icons.tsx';
// @ts-ignore
...
const TeacherPanel: React.FC<{ user: User }> = ({ user }) => {
    ...
    const handleDownloadReport = async (onlyAproved: boolean = false) => {
        ...
        try {
            ...
            if (onlyAproved) {
                filteredGrades = studentGrades.filter(g => (g.titulo_asignacion || '').toLowerCase().includes('final') && g.puntuacion >= 70);
            }
    ...
    const handleUpdateStudent = async () => {
        ...
        setIsSaving(false);
        alert("Perfil de alumno actualizado.");
    };
    ...
    const handleUpdateCourse = async () => {
        ...
        setIsSaving(false);
        if (!error) { setEditingCourse(null); fetchCourses(); alert('Materia actualizada.'); }
    };
        ...
        <div className="space-y-6 pb-20 max-w-[1600px] mx-auto px-4">
                        ...
                        <div className="bg-white dark:bg-gray-800 p-8 rounded-[2.5rem] shadow-xl border-t-8 border-green-500 h-[600px] flex flex-col">
                            ...
                            <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar flex-1">
                                {adminCourses.map(c => {
                                    ...
                                    const isAproved = studentGrades.some(g => g.curso_id === c.id && (g.titulo_asignacion || '').toLowerCase().includes('final') && g.puntuacion >= 70);
                                    return (
                                        <div 
                                            key={c.id} onClick={() => toggleInscription(c.id)}
                                        ...
                                        >
                                            ...
                                            <div className={`h-10 w-10 rounded-2xl flex items-center justify-center transition-all ${isInscribed ? 'bg-blue-500 text-white shadow-lg' : 'bg-gray-200 text-gray-400 group-hover:scale-110'}`}>
                                                {isInscribed ? <CheckIcon className="w-6 h-6"/> : <PlusIcon className="w-5 h-5"/>}
                                            </div>
                                        </div>
                                    );
                                })}
                        ...
                        <div className="bg-white dark:bg-gray-800 p-10 rounded-[3rem] shadow-2xl border-t-8 border-blue-500 space-y-8 animate-scale-in">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="md:col-span-2">
                                    <label className="text-[10px] font-black text-blue-600 block mb-3 uppercase tracking-widest">Enlace Miniatura (URL de Supabase)</label>
                                    <input type="text" value={editingCourse.image_url || ''} onChange={e => setEditingCourse({...editingCourse, image_url: e.target.value})} className="w-full p-4 border border-gray-100 rounded-2xl dark:bg-gray-700 shadow-inner font-bold" />
                                    {editingCourse.image_url && <div className="mt-6 h-40 w-72 overflow-hidden rounded-3xl shadow-2xl border-4 border-white"><img src={editingCourse.image_url} className="w-full h-full object-cover" alt="Preview"/></div>}
                                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
                    <div className="bg-white dark:bg-gray-800 p-10 rounded-[3rem] shadow-2xl border-t-8 border-blue-500 h-fit space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <input type="text" value={newAssignTitle} onChange={e => setNewAssignTitle(e.target.value)} placeholder="Nombre de la tarea" className="w-full p-4 text-xs border border-gray-100 rounded-2xl dark:bg-gray-700 font-bold"/>
                            <input type="date" value={newAssignDate} onChange={e => setNewAssignDate(e.target.value)} className="w-full p-4 text-xs border border-gray-100 rounded-2xl dark:bg-gray-700 font-bold"/>
                            <button onClick={handleAddAssignment} className="w-full bg-blue-600 text-white font-black py-4 rounded-2xl text-[10px] uppercase shadow-2xl hover:bg-blue-700 tracking-widest transition-all">Publicar Tarea</button>
                        </div>
                    </div>
                    <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-2xl overflow-hidden border dark:border-gray-700"><table className="w-full"><thead className="bg-gray-50 dark:bg-gray-900 text-[10px] font-black uppercase text-gray-400 tracking-widest"><tr><th className="px-8 py-5 text-left">Tarea</th><th className="px-8 py-5">Vence</th><th className="px-8 py-5"></th></tr></thead><tbody className="divide-y dark:divide-gray-700">
                        {assignments.map(a => (<tr key={a.id} className="text-xs hover:bg-gray-50 transition-all"><td className="px-8 py-5 font-black text-gray-700 dark:text-gray-200">{a.titulo}</td><td className="px-8 py-5 text-gray-400 font-bold">{a.fecha_entrega || 'N/A'}</td><td className="px-8 py-5 text-right"><button onClick={async () => { if(confirm('¿Eliminar tarea?')) { await supabase.from('asignaciones').delete().eq('id', a.id); fetchAssignments(); } }} className="text-red-300 hover:text-red-600 p-2 transition-all"><TrashIcon className="w-5 h-5"/></button></td></tr>))}
                    </tbody></table></div>
                </div>
            )}
            ...
            {activeTab === 'exams' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
                    <div className="bg-white dark:bg-gray-800 p-10 rounded-[3rem] shadow-2xl border-t-8 border-red-500 h-fit space-y-6">
                        ...
                        <div className="space-y-4">
                            <select value={newExamCourse} onChange={e => setNewExamCourse(e.target.value)} className="w-full p-4 text-xs border border-gray-100 rounded-2xl dark:bg-gray-700 font-black uppercase shadow-sm"><option value="">Materia...</option>{adminCourses.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select>
                            <input type="text" value={newExamTitle} onChange={e => setNewExamTitle(e.target.value)} placeholder="Ej: Examen Parcial I" className="w-full p-4 text-xs border border-gray-100 rounded-2xl dark:bg-gray-700 font-bold"/>
                            <input type="date" value={newExamDate} onChange={e => setNewExamDate(e.target.value)} className="w-full p-4 text-xs border border-gray-100 rounded-2xl dark:bg-gray-700 font-bold"/>
                            <button onClick={handleAddExam} className="w-full bg-red-600 text-white font-black py-4 rounded-2xl text-[10px] uppercase shadow-2xl hover:bg-red-700 tracking-widest transition-all">Crear Examen</button>
                        </div>
                    </div>
                    <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-2xl overflow-hidden border dark:border-gray-700"><table className="w-full"><thead className="bg-gray-50 dark:bg-gray-900 text-[10px] font-black uppercase text-gray-400 tracking-widest"><tr><th className="px-8 py-5 text-left">Examen</th><th className="px-8 py-5">Fecha</th><th className="px-8 py-5"></th></tr></thead><tbody className="divide-y dark:divide-gray-700">
                        {exams.map(e => (<tr key={e.id} className="text-xs hover:bg-gray-50 transition-all"><td className="px-8 py-5 font-black text-gray-700 dark:text-gray-200">{e.titulo}</td><td className="px-8 py-5 text-gray-400 font-bold">{e.fecha || 'Sin fecha'}</td><td className="px-8 py-5 text-right"><button onClick={async () => { if(confirm('¿Eliminar examen?')) { await supabase.from('examenes').delete().eq('id', e.id); fetchExams(); } }} className="text-red-300 hover:text-red-600 p-2 transition-all"><TrashIcon className="w-5 h-5"/></button></td></tr>))}
                    </tbody></table></div>
                </div>
            )}
            ...
            {activeTab === 'attendance' && (
                <div className="bg-white dark:bg-gray-800 p-10 rounded-[3rem] shadow-2xl border-t-8 border-blue-500 space-y-8 animate-fade-in">
                    ...
                    <div className="flex flex-col md:flex-row gap-6 p-6 bg-gray-50 dark:bg-gray-900/50 rounded-[2.5rem] border-2 border-dashed border-gray-200">
                        <select value={attendanceCourse} onChange={e => setAttendanceCourse(e.target.value)} className="flex-1 p-4 text-xs border-none rounded-2xl bg-white dark:bg-gray-700 font-black uppercase shadow-lg"><option value="">Materia...</option>{adminCourses.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select>
                        <input type="date" value={attendanceDate} onChange={e => setAttendanceDate(e.target.value)} className="p-4 text-xs border-none rounded-2xl bg-white dark:bg-gray-700 font-black shadow-lg"/>
                        <button onClick={loadAttendance} className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-black text-[10px] uppercase shadow-2xl hover:bg-blue-700 tracking-widest transition-all">Cargar Lista</button>
                    </div>
                    {attendanceCourse && (
                        <div className="overflow-hidden rounded-[2.5rem] border border-gray-100 dark:border-gray-700"><table className="w-full text-left"><thead className="bg-gray-50 dark:bg-gray-900 text-[10px] font-black uppercase text-gray-400 tracking-widest"><tr><th className="px-8 py-5">Nombre</th><th className="px-8 py-5 text-center">Estado</th></tr></thead><tbody className="divide-y dark:divide-gray-700">
                            {students.map(s => (
                                <tr key={s.id} className="text-sm hover:bg-gray-50/50 transition-all"><td className="px-8 py-5 font-bold text-gray-700 dark:text-gray-200">{s.nombre}</td><td className="px-8 py-5"><div className="flex justify-center gap-3">
                                    {['presente', 'ausente', 'tarde'].map(st => (
                                        <button key={st} onClick={() => markAttendance(s.id, st)} className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase transition-all shadow-md ${attendanceMap[s.id] === st ? (st==='presente'?'bg-green-600 text-white scale-110 shadow-green-200':st==='ausente'?'bg-red-600 text-white scale-110 shadow-red-200':'bg-amber-500 text-white scale-110 shadow-amber-200') : 'bg-gray-100 text-gray-400 dark:bg-gray-800'}`}>{st}</button>
                                    ))}
                                </div></td></tr>
                            ))}
                        </tbody></table></div>
