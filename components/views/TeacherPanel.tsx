...
const TeacherPanel: React.FC<{ user: User }> = ({ user }) => {
    ...
    const handleSendCreds = async () => {
        ...
        try {
            ...
            else alert("No se pudo enviar el correo.");
    ...
    const handleDownloadPDF = () => {
        ...
        doc.setFontSize(14); doc.text("Boletín de Calificaciones", 45, 33);
        ...
        autoTable(doc, { 
            startY: 70, head: [['Materia', 'Actividad', 'Nota']], body: tableData,
    ...
    const handlePostAnnouncement = async () => {
        if (!newItem.title || !newItem.content) return alert("Completa los campos.");
        setIsSaving(true);
        const { error } = await supabase.from('mensajes').insert({ 
            remitente: user.name, asunto: newItem.title, contenido: newItem.content, 
            leido: false, fecha_envio: new Date().toISOString() 
        });
        if (!error) alert("Anuncio publicado.");
        setNewItem({ courseId: '', title: '', date: '', time: '', content: '', type: 'pdf' });
        fetchAnnouncements();
        setIsSaving(false);
    };

    if (loading) return <div className="p-20 text-center animate-pulse text-blue-600 font-black tracking-widest uppercase">Cargando Panel de Administración...</div>;
    ...
    return (
        <div className="space-y-6 pb-20 max-w-[1600px] mx-auto px-4 animate-fade-in">
                 ...
                 <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-2xl overflow-hidden border dark:border-gray-700 animate-fade-in">
                    <div className="p-6 border-b dark:border-gray-700 flex flex-col md:flex-row justify-between items-center gap-4 bg-gray-50/50">
                        <div className="relative w-full max-w-lg group">
                            <input type="text" placeholder="Buscar por nombre..." value={studentSearchTerm} onChange={(e) => setStudentSearchTerm(e.target.value)} className="w-full pl-12 pr-4 py-4 rounded-2xl bg-white dark:bg-gray-700 shadow-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead><tr className="bg-gray-100 dark:bg-gray-900 text-[10px] uppercase text-gray-400 font-black tracking-widest"><th className="px-8 py-5">Estudiante</th><th className="px-8 py-5">Rol</th><th className="px-8 py-5">Estado</th><th className="px-8 py-5 text-right">Acciones</th></tr></thead>
                                ...
                                {students.filter(s => s.nombre.toLowerCase().includes(studentSearchTerm.toLowerCase())).map(s => (
                                    <tr key={s.id} className="hover:bg-blue-50/30 transition-colors group">
                                        <td className="px-8 py-5 flex items-center font-bold text-sm text-gray-800 dark:text-gray-200">
                                            <img src={s.avatar_url} className="w-12 h-12 rounded-2xl mr-5 shadow-md border-2 border-white group-hover:scale-110 transition-transform"/>
                                            {s.nombre}
                                        </td>
                                        <td className="px-8 py-5 text-xs text-blue-500 font-black uppercase">{s.rol}</td>
                                        <td className="px-8 py-5"><span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase ${s.activo ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{s.activo ? 'Activo' : 'Inactivo'}</span></td>
                                        <td className="px-8 py-5 text-right"><button onClick={() => handleSelectStudent(s)} className="bg-blue-600 text-white px-6 py-2.5 rounded-xl text-[10px] font-black uppercase hover:bg-blue-700 tracking-widest shadow-lg transition-all">Gestionar</button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
                    <div className="bg-white dark:bg-gray-800 w-full max-w-lg rounded-[3.5rem] shadow-2xl overflow-hidden border dark:border-gray-700">
                        <div className="bg-blue-600 p-8 text-white flex justify-between items-center">
                            <div><h3 className="text-xl font-black uppercase tracking-tighter">Registrar Miembro</h3><p className="text-blue-100 text-[10px] font-bold uppercase tracking-widest">LTS Academic Cloud</p></div>
                        ...
                        <div className="p-10 space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Estado</label><select value={newMember.activo ? 'true' : 'false'} onChange={e => setNewMember({...newMember, activo: e.target.value === 'true'})} className="w-full p-4 rounded-2xl bg-gray-50 font-black uppercase"><option value="true">Activo</option><option value="false">Inactivo</option></select></div>
                            </div>
                            <button onClick={handleCreateMember} disabled={isSaving} className={`w-full py-5 mt-4 rounded-[2rem] font-black text-[11px] uppercase shadow-xl tracking-widest transition-all ${isSaving ? 'bg-gray-400' : 'bg-blue-600 text-white hover:bg-blue-700 active:scale-95'}`}>Guardar Cuenta</button>
                        </div>
                    </div>
                </div>
            )}

            {/* DETALLE ESTUDIANTE (FICHA) */}
            {activeTab === 'students' && selectedStudent && (
                <div className="animate-fade-in space-y-6">
                    <div className="flex justify-between items-center bg-gray-100 p-4 rounded-3xl border border-gray-200 shadow-sm">
                        ...
                        <button onClick={handleDownloadPDF} className="bg-white text-gray-700 border px-6 py-2.5 rounded-xl text-[10px] font-black uppercase flex items-center hover:bg-gray-50 transition-all shadow-sm active:scale-95"><DownloadIcon className="w-4 h-4 mr-2 text-blue-600"/> Descargar Boletín de Notas</button>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                        {/* Perfil del Alumno */}
                        <div className="bg-white dark:bg-gray-800 p-8 rounded-[2.5rem] shadow-xl border-t-8 border-blue-600 text-center flex flex-col">
                            <div className="relative inline-block mx-auto mb-4">
                                <img src={selectedStudent.avatar_url} className="w-28 h-28 rounded-[2.5rem] shadow-xl border-4 border-white object-cover"/>
                                <div onClick={() => setEditActivo(!editActivo)} className={`absolute -bottom-2 -right-2 p-1.5 rounded-full border-4 border-white shadow-lg cursor-pointer ${editActivo ? 'bg-green-500' : 'bg-red-500'}`}>
                                    {editActivo ? <CheckIcon className="w-4 h-4 text-white"/> : <XIcon className="w-4 h-4 text-white"/>}
                                </div>
                            </div>
                            <h4 className="font-black text-gray-800 dark:text-white text-lg leading-tight mb-1 uppercase tracking-tighter">{selectedStudent.nombre}</h4>
                            <p className="text-[9px] font-black text-blue-500 uppercase tracking-widest mb-6">{selectedStudent.rol}</p>
                            ...
                            <div className="mt-8 space-y-3">
                                ...
                                <button onClick={handleUpdateProfile} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase shadow-lg hover:bg-blue-700 transition-all active:scale-95">Guardar Perfil</button>
                            </div>
                        </div>

                        {/* Subir Notas y Plan de Estudios */}
                        <div className="bg-white dark:bg-gray-800 p-8 rounded-[2.5rem] shadow-xl border-t-8 border-green-500">
                             <h3 className="text-[10px] font-black uppercase mb-6 flex items-center text-gray-400 tracking-widest"><ChartBarIcon className="w-4 h-4 mr-2"/> Calificaciones y Plan</h3>
                             
                             <div className="bg-gray-50 dark:bg-gray-900/50 p-4 rounded-3xl border mb-6 space-y-3 shadow-inner">
                                <p className="text-[9px] font-black uppercase text-center text-gray-400">Subir Nueva Nota</p>
                                <select value={newGrade.courseId} onChange={e => setNewGrade({...newGrade, courseId: e.target.value})} className="w-full p-3 text-[10px] rounded-xl font-black uppercase bg-white border-none shadow-sm"><option value="">Seleccionar Materia...</option>{adminCourses.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select>
                                <input placeholder="Actividad (Ej: Examen Final)" value={newGrade.title} onChange={e => setNewGrade({...newGrade, title: e.target.value})} className="w-full p-3 text-xs rounded-xl bg-white border-none shadow-sm"/>
                                <div className="flex gap-2"><input type="number" placeholder="Nota" value={newGrade.score} onChange={e => setNewGrade({...newGrade, score: e.target.value})} className="flex-1 p-3 text-xs rounded-xl bg-white border-none shadow-sm"/><button onClick={handleAddGrade} className="bg-green-600 text-white p-3 rounded-xl hover:bg-green-700 transition-all"><PlusIcon className="w-5 h-5"/></button></div>
                             </div>
                                ...
                                {adminCourses.map(course => {
                                    ...
                                    return (
                                        <div key={course.id} onClick={() => toggleInscription(course.id)} className={`flex justify-between items-center p-4 rounded-2xl border-2 transition-all cursor-pointer ${isEnrolled ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-100 hover:border-blue-200'}`}>
                                            ...
                                            <div className={`p-2 rounded-xl ${isEnrolled ? 'bg-green-600 text-white shadow-md' : 'bg-white text-gray-300'}`}><CheckIcon className="w-3 h-3"/></div>
                                        </div>
                                    );
                                })}
                            </div>
                        ...
                        {/* Listado de Notas Actuales */}
                        <div className="bg-white dark:bg-gray-800 p-8 rounded-[2.5rem] shadow-xl border-t-8 border-amber-500">
                             <h3 className="text-[10px] font-black uppercase mb-6 flex items-center text-gray-400 tracking-widest"><ClipboardListIcon className="w-4 h-4 mr-2"/> Notas Registradas</h3>
                             <div className="space-y-3 overflow-y-auto max-h-[500px] pr-2">
                                {studentGrades.map(g => (
                                    <div key={g.id} className="p-4 bg-gray-50 dark:bg-gray-900/40 rounded-2xl flex justify-between items-center group shadow-sm transition-all hover:bg-white">
                                        <div className="truncate text-left"><p className="text-[9px] font-black text-blue-500 uppercase">{(adminCourses.find(c => c.id === g.curso_id)?.nombre || 'General').substring(0,18)}</p><p className="text-xs font-bold truncate text-gray-800 dark:text-gray-200">{g.titulo_asignacion}</p></div>
                                ...
                                {studentGrades.length === 0 && <p className="text-center text-[10px] text-gray-400 uppercase py-20 font-black opacity-30">Sin notas aún</p>}
                            </div>
                        </div>

                        {/* Pagos y Finanzas */}
                        <div className="bg-white dark:bg-gray-800 p-8 rounded-[2.5rem] shadow-xl border-t-8 border-indigo-500">
                             <h3 className="text-[10px] font-black uppercase mb-6 flex items-center text-gray-400 tracking-widest"><CurrencyDollarIcon className="w-4 h-4 mr-2"/> Pagos y Deuda</h3>
                             
                             <div className="bg-indigo-50 dark:bg-indigo-900/30 p-4 rounded-3xl border mb-6 space-y-3 shadow-inner border-indigo-100">
                                <p className="text-[9px] font-black uppercase text-center text-indigo-500">Registrar Nuevo Abono</p>
                                <input type="number" placeholder="Monto en USD ($)" value={newPayment.amount} onChange={e => setNewPayment({...newPayment, amount: e.target.value})} className="w-full p-3 text-xs rounded-xl bg-white border-none shadow-sm"/>
                                <select value={newPayment.method} onChange={e => setNewPayment({...newPayment, method: e.target.value})} className="w-full p-3 text-[10px] rounded-xl font-black bg-white border-none shadow-sm uppercase"><option value="Zelle">Zelle</option><option value="Pago Móvil">Pago Móvil</option><option value="Efectivo">Efectivo</option></select>
                                <button onClick={handleAddPayment} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-black text-[10px] uppercase shadow-lg hover:bg-indigo-700 transition-all active:scale-95">Registrar Pago</button>
                             </div>

                             <div className="space-y-3 overflow-y-auto max-h-[300px] pr-2">
                                {studentPayments.map(p => (
                                    <div key={p.id} className="p-4 bg-gray-50 dark:bg-gray-900/40 rounded-2xl flex justify-between items-center group shadow-sm hover:bg-white transition-all">
                                        <div className="truncate text-left"><p className="text-[9px] font-black text-gray-400 uppercase">{p.date}</p><p className="text-xs font-bold truncate text-gray-800 dark:text-gray-200">{p.method}</p></div>
                                        <div className="flex items-center space-x-2"><span className="text-green-600 text-xs font-black">${p.amount}</span><button onClick={() => handleDeleteItem('pagos', p.id)} className="text-gray-300 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100"><TrashIcon className="w-4 h-4"/></button></div>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            {/* --- SECCIÓN MATERIAS (CON LUPA Y PREVIEW) --- */}
                ...
                <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-2xl overflow-hidden border dark:border-gray-700 animate-fade-in">
                    <div className="p-8 border-b dark:border-gray-700 flex justify-between items-center bg-gray-50/50">
                        <div className="relative w-full max-w-lg group">
                            ...
                            <input type="text" placeholder="Buscar materia por nombre o código..." value={courseSearchTerm} onChange={(e) => setCourseSearchTerm(e.target.value)} className="w-full pl-14 pr-6 py-4 rounded-2xl bg-white dark:bg-gray-700 shadow-xl text-sm border-2 border-transparent focus:border-blue-500 outline-none transition-all dark:text-white" />
                        </div>
                    </div>
                    <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="bg-gray-50 dark:bg-gray-900 text-[10px] uppercase font-black tracking-widest text-gray-400"><th className="px-10 py-6">Materia</th><th className="px-10 py-6">Profesor</th><th className="px-10 py-6 text-right">Acciones</th></tr></thead><tbody className="divide-y dark:divide-gray-700">{adminCourses.filter(c => c.nombre.toLowerCase().includes(courseSearchTerm.toLowerCase())).map(c => (<tr key={c.id} className="hover:bg-blue-50/40 group"><td className="px-10 py-6 font-bold text-sm text-gray-800 dark:text-gray-200">{c.nombre} <br/> <span className="text-[9px] text-blue-500 uppercase tracking-tighter">ID: {c.id}</span></td><td className="px-10 py-6 text-sm text-gray-600 font-medium">{c.profesor}</td><td className="px-10 py-6 text-right"><button onClick={() => setSelectedCourse(c)} className="bg-blue-600 text-white px-8 py-3 rounded-2xl text-[10px] font-black uppercase hover:bg-blue-700 tracking-widest shadow-lg active:scale-95 transition-all">Editar Ficha</button></td></tr>))}</tbody></table></div>
                </div>
            )}
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className="flex gap-4 items-center">
                                        ...
                                        <div className="w-20 h-20 bg-gray-100 rounded-2xl shadow-lg overflow-hidden border-4 border-white flex-shrink-0 flex items-center justify-center">
                                            {selectedCourse.image_url ? <img src={selectedCourse.image_url} className="w-full h-full object-cover" /> : <div className="text-[8px] text-gray-400 font-black p-2 text-center">SIN IMAGEN</div>}
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="space-y-6">
                                <div className="space-y-1"><label className="text-[10px] font-black uppercase text-gray-400 ml-1 tracking-widest">Descripción General</label><textarea rows={3} value={selectedCourse.description} onChange={e => setSelectedCourse({...selectedCourse, description: e.target.value})} className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none shadow-inner text-sm font-medium"></textarea></div>
                                <div className="space-y-1"><label className="text-[10px] font-black uppercase text-gray-400 ml-1 tracking-widest">Contenido Académico Detallado</label><textarea rows={6} value={selectedCourse.detailedContent} onChange={e => setSelectedCourse({...selectedCourse, detailedContent: e.target.value})} className="w-full p-4 rounded-2xl bg-gray-50 border-none outline-none shadow-inner text-sm font-medium"></textarea></div>
                            </div>
                        </div>
            ...
            {activeTab === 'resources' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
                    ...
                    <div className="bg-white dark:bg-gray-800 p-10 rounded-[3rem] shadow-2xl border-t-8 border-indigo-500 h-fit space-y-8">
                        <h3 className="font-black flex items-center text-gray-700 dark:text-white uppercase text-sm tracking-widest"><PlusIcon className="w-6 h-6 mr-3 text-indigo-600"/> Publicar Nuevo Material</h3>
                        <div className="space-y-5 text-left">
                            ...
                            <div className="space-y-1"><label className="text-[9px] font-black uppercase text-gray-400 ml-2 tracking-widest">Título del Material</label><input type="text" value={newItem.title} onChange={e => setNewItem({...newItem, title: e.target.value})} placeholder="Ej: Guía de Lectura Unidad 2" className="w-full p-4 rounded-2xl bg-gray-50 font-bold border-none outline-none shadow-inner text-sm"/></div>
                            ...
                            {newItem.type === 'link' ? (
                                ...
                                <div onClick={() => fileInputRef.current?.click()} className={`w-full p-8 border-2 border-dashed rounded-[2.5rem] cursor-pointer text-center group transition-all ${selectedFile ? 'border-green-400 bg-green-50' : 'border-gray-200 bg-gray-50 hover:bg-white hover:border-indigo-500'}`}>
                                    ...
                                    <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{selectedFile ? selectedFile.name : 'Haz clic para seleccionar archivo'}</p>
                                    {selectedFile && <p className="text-[8px] text-green-500 mt-2 font-bold uppercase tracking-tighter">Archivo Cargado Localmente</p>}
                                </div>
                            )}
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-[3rem] shadow-2xl overflow-hidden h-[750px] flex flex-col border dark:border-gray-700">
                        <div className="p-8 bg-gray-100 dark:bg-gray-900 border-b flex justify-between items-center"><h3 className="font-black text-[11px] uppercase text-gray-400 tracking-widest">Biblioteca Digital de Recursos</h3><span className="bg-indigo-600 text-white text-[9px] font-black px-4 py-1.5 rounded-full shadow-lg">{courseResources.length} ITEMS</span></div>
                        <div className="flex-1 overflow-y-auto divide-y dark:divide-gray-700">
                                ...
                                <div key={res.id} className="p-6 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-all">
                                    <div className="flex items-center truncate text-left">
                                        ...
                                        <div className="truncate">
                                            ...
                                            <p className="text-[9px] text-gray-400 font-black uppercase mt-1">{(adminCourses.find(c => c.id === res.courseId)?.nombre || 'LTS General')}</p>
                                        </div>
                                    </div>
                            ...
                            )) : <div className="p-24 text-center opacity-30 flex flex-col items-center justify-center"><LinkIcon className="w-20 h-20 mb-4"/><p className="font-black text-sm uppercase tracking-widest">Sin archivos registrados</p></div>}
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="bg-white dark:bg-gray-800 p-10 rounded-[3.5rem] shadow-2xl border-t-8 border-green-500 space-y-10 animate-fade-in">
                    <div className="flex flex-col md:flex-row gap-6 items-end bg-gray-50 dark:bg-gray-900/50 p-8 rounded-[3rem] border dark:border-gray-700 shadow-inner">
                        <div className="flex-1 space-y-2 text-left">
                            <label className="text-[10px] font-black uppercase text-gray-400 ml-1 tracking-widest">Seleccionar Cátedra</label>
                        ...
                        <div className="w-full md:w-64 space-y-2 text-left">
                            <label className="text-[10px] font-black uppercase text-gray-400 ml-1 tracking-widest">Fecha de Registro</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {attList.length > 0 ? attList.map(alumno => (
                            <div key={alumno.id} onClick={() => cycleAttendance(alumno.id, alumno.estado)} className={`flex items-center justify-between p-6 rounded-[2.5rem] cursor-pointer transition-all border-2 shadow-sm transform hover:-translate-y-2 ${alumno.estado === 'presente' ? 'bg-green-50 border-green-200' : alumno.estado === 'ausente' ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-100 dark:bg-gray-700/50'}`}>
                                <div className="flex items-center text-left">
                                    ...
                                    <div><p className="text-xs font-black text-gray-800 dark:text-white leading-tight">{alumno.nombre}</p><p className={`text-[8px] font-black uppercase mt-1 ${alumno.estado === 'ninguno' ? 'text-gray-400' : 'text-blue-500'}`}>{alumno.estado === 'ninguno' ? 'Pendiente' : alumno.estado}</p></div>
                                </div>
                        ...
                        )) : <div className="col-span-full py-24 text-center opacity-30 text-sm font-black uppercase flex flex-col items-center"><UserGroupIcon className="w-20 h-20 mb-4"/><p>Selecciona materia y lista el grupo para marcar asistencia</p></div>}
                    </div>
                </div>
            )}
            ...
            {activeTab === 'announcements' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
                    <div className="bg-white dark:bg-gray-800 p-10 rounded-[3rem] shadow-2xl border-t-8 border-indigo-500 h-fit space-y-8">
                        <h3 className="font-black flex items-center text-gray-700 dark:text-white uppercase text-sm tracking-widest"><PlusIcon className="w-6 h-6 mr-3 text-indigo-600"/> Redactar Aviso Académico</h3>
                        <div className="space-y-4 text-left">
                            <input type="text" value={newItem.title} onChange={e => setNewItem({...newItem, title: e.target.value})} placeholder="Asunto del Anuncio..." className="w-full p-5 text-sm bg-gray-50 dark:bg-gray-900/50 border-none rounded-[2rem] font-black shadow-inner outline-none dark:text-white"/>
                            <textarea rows={5} value={newItem.content} onChange={e => setNewItem({...newItem, content: e.target.value})} placeholder="Escribe aquí el cuerpo del anuncio detallado para los estudiantes..." className="w-full p-6 text-sm bg-gray-50 dark:bg-gray-900/50 border-none rounded-[2rem] font-medium shadow-inner outline-none dark:text-white"></textarea>
                            <button onClick={handlePostAnnouncement} disabled={isSaving} className={`w-full bg-indigo-600 text-white py-6 rounded-[2.5rem] font-black text-[11px] uppercase shadow-2xl hover:bg-indigo-700 flex items-center justify-center transition-all ${isSaving ? 'opacity-50' : 'active:scale-95'}`}><SendIcon className="w-5 h-5 mr-3"/> Publicar en Tablón</button>
                        </div>
                    </div>
                    <div className="bg-white dark:bg-gray-800 rounded-[3rem] shadow-2xl overflow-hidden h-[750px] flex flex-col border dark:border-gray-700">
                        <div className="p-8 bg-gray-100 dark:bg-gray-900 border-b font-black text-[11px] uppercase text-gray-400 tracking-widest">Historial de Anuncios Enviados</div>
                        <div className="flex-1 overflow-y-auto divide-y dark:divide-gray-700">
                            ...
                            )) : <div className="p-24 text-center opacity-30 flex flex-col items-center justify-center"><MailIcon className="w-20 h-20 mb-4"/><p className="font-black text-sm uppercase tracking-widest">Sin anuncios registrados</p></div>}
                        </div>
                    </div>
                </div>
            )}
            ...
            {(activeTab === 'assignments' || activeTab === 'exams') && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
                    <div className="bg-white dark:bg-gray-800 p-10 rounded-[3rem] shadow-2xl border-t-8 border-blue-500 h-fit space-y-8">
                        <h3 className="font-black flex items-center text-gray-700 dark:text-white uppercase text-sm tracking-widest"><PlusIcon className="w-6 h-6 mr-3 text-blue-600"/> Nuevo {activeTab === 'assignments' ? 'Tarea' : 'Examen'}</h3>
                        <div className="space-y-5 text-left">
                            <select value={newItem.courseId} onChange={e => setNewItem({...newItem, courseId: e.target.value})} className="w-full p-4 rounded-2xl bg-gray-50 dark:bg-gray-900 border-none outline-none shadow-inner font-black uppercase text-sm">{adminCourses.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select>
                            <input type="text" value={newItem.title} onChange={e => setNewItem({...newItem, title: e.target.value})} placeholder="Título de la actividad..." className="w-full p-4 rounded-2xl bg-gray-50 dark:bg-gray-900 font-bold border-none outline-none shadow-inner"/>
                            <div className="flex gap-4"><input type="date" value={newItem.date} onChange={e => setNewItem({...newItem, date: e.target.value})} className="flex-1 p-4 rounded-2xl bg-gray-50 dark:bg-gray-900 font-bold border-none outline-none shadow-inner"/>{activeTab === 'exams' && <input type="time" value={newItem.time} onChange={e => setNewItem({...newItem, time: e.target.value})} className="w-32 p-4 rounded-2xl bg-gray-50 dark:bg-gray-900 font-bold border-none outline-none shadow-inner"/>}</div>
                            <button onClick={activeTab === 'assignments' ? async () => { await supabase.from('asignaciones').insert({ curso_id: newItem.courseId, titulo: newItem.title, fecha_entrega: newItem.date, entregado: false }); fetchAssignments(); alert("Tarea publicada."); } : async () => { await supabase.from('examenes').insert({ curso_id: newItem.courseId, titulo: newItem.title, fecha: newItem.date, hora: newItem.time }); fetchExams(); alert("Examen publicado."); }} className="w-full bg-blue-600 text-white py-5 rounded-[2rem] font-black text-[11px] uppercase hover:bg-blue-700 shadow-xl transition-all active:scale-95">Publicar en Agenda</button>
                        </div>
                    </div>
                    <div className="bg-white dark:bg-gray-800 rounded-[3rem] shadow-2xl overflow-hidden h-[750px] flex flex-col border dark:border-gray-700">
                        <div className="p-8 bg-gray-100 dark:bg-gray-900 border-b font-black text-[11px] uppercase text-gray-400 tracking-widest">Cronograma de Actividades</div>
                        <div className="flex-1 overflow-y-auto divide-y dark:divide-gray-700">
                            {(activeTab === 'assignments' ? allAssignments : allExams).map((item: any) => (
                                <div key={item.id} className="p-8 hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-all flex justify-between items-center">
                                    <div className="text-left truncate"><p className="text-[10px] font-black text-blue-500 uppercase tracking-widest">{(adminCourses.find(c => c.id === item.curso_id)?.nombre || 'LTS')}</p><p className="text-sm font-black text-gray-800 dark:text-white truncate">{item.titulo}</p><p className="text-[10px] text-gray-400 mt-2 font-bold uppercase tracking-widest flex items-center"><CalendarIcon className="w-3.5 h-3.5 mr-1.5 inline"/> {item.fecha_entrega || item.fecha} {item.hora && `- ${item.hora}`}</p></div>
